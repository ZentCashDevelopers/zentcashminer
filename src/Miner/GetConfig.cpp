...
Pool getPool()
{
    ...
    while (true)
    {
        std::cout << InformationMsg("Enter the pool address to mine to.") << std::endl
                  << InformationMsg("This should look something like pool.zent.cash:11153: ");
        ...
        {
            std::cout << WarningMsg("Invalid pool address! Should be in the form host:port, for example, pool.zent.cash:11153!")
                      << std::endl;
...
MinerConfig getMinerConfig(int argc, char **argv)
{
    ...
    try
    {
        ...
        if (version)
        {
            std::cout << "zentcashminer " << Constants::VERSION << std::endl;
            exit(0);
        }
